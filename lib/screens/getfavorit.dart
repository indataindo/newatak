...
class Getfavorit extends StatefulWidget {
  const Getfavorit({Key key}) : super(key: key);

  @override
  State<Getfavorit> createState() => _GetfavoritState();
}

class _GetfavoritState extends State<Getfavorit> {
  ...
  tampil() async {
    ...
    final response = await http.post(Uri.parse(RestApi.getfavorit), body: {
      "iduser": idku,
    });
    if (response.contentLength == 2) {
      ...
      datapromo.forEach((api) {
        final p =
            new Modepolygonfavorit(api['idpolygon'], api['id'], api['title']);
        listarea.add(p);
      });
  ...
  _hapus(int id) async {
    final response = await http.post(Uri.parse(RestApi.deletefavorit), body: {
      "id": id.toString(),
    });
    final data = jsonDecode(response.body);
    print(data);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      child: loading
          ? Center(child: Text("No Favorite Area"))
          : ListView.builder(
